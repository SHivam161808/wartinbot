...
with context management, improved tone, and bug fixes.
"""
...
# ─────────────────────────────────────────────────────────────
# System prompt – improved tone, address, policies
# ─────────────────────────────────────────────────────────────
SYSTEM_TEMPLATE = """\
...
# ─────────────────────────────────────────────────────────────
# SessionTerminator with expanded goodbye phrases
# ─────────────────────────────────────────────────────────────
_BYE_PHRASES = [
...
class SessionTerminator(FrameProcessor):
    ...
    async def trigger_end(self):
        ...
        await asyncio.sleep(1.0)  # reduced from 4s to end faster
...
# ─────────────────────────────────────────────────────────────
# Bot entry point with context trimming
# ─────────────────────────────────────────────────────────────
MAX_CONVERSATION_TURNS = 12  # keep last 30 messages (system + ~15 exchanges)
...
async def run_bot(
    ...
    # Transport with VAD sensitivity
    ...
    llm = GroqLLMService(api_key=os.environ["GROQ_API_KEY"], model="llama-3.1-8b-instant", temperature=0.65, max_tokens=300)
    ...
    context = OpenAILLMContext(messages)   # keep system + last 9 messages (~4 exchanges)
    ...
    def trim_context():
        ...
        messages[:] = new_msgs
        # IMPORTANT: Update the context used by the LLM aggregator
    ...
    async def on_transcript(transport, message):
        ...
        # Trim context to prevent token bloat
        trim_context()
